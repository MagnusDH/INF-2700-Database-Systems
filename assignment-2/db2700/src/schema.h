...
/*Prints info about a field (type, length, offset, next field)
-DONE*/
extern void put_field_info(pmsg_level level, field_desc_p f);

/*
  -Puts info from record r, into pmsg_level level
  -Put "record" into message line
  -if table field is int: put record[0] into schema
  -if table field is str: put record[0] into schema
  -put "|" into message line
-DONE*/
...
/*
-Prints error if schema does not exists
-Prints name of schema, it's number of field(columns) and total bytes
-DONE
*/
...
/** Return an existing schema, NULL if the named schema does not exist.
  -Calls get_table()
  -If get_table() returns a table, then the table->schema is returned
...
/*
  -Adds a field(column/attribute) to the given schema 

  -Return 0 if no schema is given
  -if schema is empty
  >Initialize schema_p and field_desc_p attributes
  -if schema is NOT empty
  >set schema_p and field_desc_p attributes to match length and number of fields etc...
...
/** Return an existing table desc, NULL if the table does not exist. 
  -Loops through list of tables and checks if one has the same name as argument "name"
  -If table is found, return it
  -Else return 0
-DONE*/
extern tbl_p get_table(char const* name);

/*
  -Looks for table "t" in list of tables
  -Updates the table pointers to point correct to each other
  -Closes the file that this table uses
  -renames some table->schema->name?
  -Frees memory block used by "tbl_backup"
  -Releases table->schema
  -Free memory of provided table
  -Return
DONE*/
...
extern tbl_p table_search(tbl_p t, char const* attr, char const* op, int val, int binary);
