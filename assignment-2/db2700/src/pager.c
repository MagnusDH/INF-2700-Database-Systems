...
/*
  -Puts file_handle's info to message line
  -This is put in: file_handle->file_name, fh->fd,, number of blocks, current block number to message line  
  -Return if no file handle is found
-DONE*/
...
/*-Forward declaration...
  -Searches the global_file_handles[] list and finds position of requested file
  -Returns a pointer to the file_handle
-DONE*/
static fhandle_p get_tbl_file(char const* fname);

/*
  -Fetches the pointer to the requested file_name (fname)
  -If file is not open, print error message
  -If file is open, put info about file_handle to message line:
    (file_handle->file_name, fh->fd, number of blocks, current block number to message line)
-DONE*/
...
/*
  -Put info about a page to message line.(page->current position, if it is pinned or unpinned and if it is dirty or clean)
  -Put info about page->block to message line (block->file_name, block->block_number, block->page_number)
  -If no page is found or page is blocked, return
-DONE*/
...
/*
  -Put info about block to message line (block->file_name, block->block_number, block->page_number)
...
/*
  -Puts info about fhandle into message line
  -If there are pages, put info about each page in message line
-DONE*/
...
/*
  -Puts number of pages in queue into message line
  -For each page in queue, append page number into message line
-DONE*/
static void put_pqueue_info(pmsg_level level, pqueue_p q, char const* which) {
  if (!q) {
...
/*
  -Puts info about pinned and unpinned pages into message line
-DONE*/
...
/*
  -Returns header offset for a page 
-DONE*/
...
/*
  -Places int val into page->content
-DONE*/
...
/*
  -Puts error message in message line if header_size != PAGE_HEADER_SIZE
-DONE*/
...
/*
  -Sets p->free_pos to pos-value and p->content to pos-value
-DONE*/
...
/*
  -Sets p->free_pos to header offset for page
-DONE*/
...
/*
  -If no page given, put error and return -1
  -set p->current_pos to pos
  -return p->current_pos
DONE*/
