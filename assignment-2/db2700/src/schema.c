...
static int less_than(int x, int y)
{
  return x > y;
}
...
static int bigger_than(int x, int y)
{
  return x < y;
}

static int biggerOrEqual(int x, int y)
{
  return x <= y;
}

static int not_equal(int x, int y)
{
    return x != y;
}

/*
  r = new record from table_search()
  s = schema to look in
  (*op) (int int) = ????
  search_value = value to look for in schema

  -find top page
  -Find bottom page
  -find middle page

  While(bottom page != top page){
    -set position to beginning of middle page
    -Get int at beginning of middle page
    -WHILE(not end of middle page, perform linear search){
      -IF(int at beginning == search value){
        -good return 0 and other stuff
      }
      ELSE{
        -increment position in page + 4
        -get new int at beginning for this new position
      }
    }
    
    (reached end of page)
    IF(search value > current int at beginning){
      bottom page = middle page + 1
    }
    ELSE{
      top page = middle page - 1
    }
  }
DONE*/
static int binary_search(record r, schema_p s, int (*op) (int, int), int search_value)
{ 
  //Find bottom page and highest page numbers
  int lowest_page_number = 0;                           //Lowest page number
  int highest_page_number = file_num_blocks(s->name);   //Highest page number 
  
  int page_current_pos = 20;
  int current_value;

  //If there is only 1 page, perform linear search in it
  if(lowest_page_number == highest_page_number){
    //Find middle page
    int middle_page_number = (lowest_page_number + highest_page_number)/2;
    page_p middle_page = get_page(s->name, middle_page_number);

    while(eop(middle_page) != 1){
      //Get first int at beginning of middle page
      current_value = page_get_int_at(middle_page, page_current_pos);
      
      //Check if this value is the value we are searching for
      if(current_value == search_value){
        page_set_current_pos(middle_page, page_current_pos);
        get_page_record(middle_page, r, s);
        return 0;
      }
      else{
        // page_current_pos += 4;
        page_current_pos += s->len;
        page_set_current_pos(middle_page, page_current_pos);
      }
    }
  }
    ...
    //Perform binary search until lowest page != highest page
    while(lowest_page_number != highest_page_number){    

      //Find middle page
      int middle_page_number = (lowest_page_number + highest_page_number)/2;
      page_p middle_page = get_page(s->name, middle_page_number);
      
      //Set position to beginning of middle page (position 20)
      page_set_current_pos(middle_page, page_current_pos);

      //Look for search value in middle page (linear search)
      while(eop(middle_page) != 1){
        //Get first int at beginning of middle page
        current_value = page_get_int_at(middle_page, page_current_pos);
      ...
      //VALUE WAS NOT IN MIDDLE PAGE
      //Reset position to beginning of middle page
      page_current_pos = 20;
      page_set_current_pos(middle_page, page_current_pos);
      
      //Get int at beginning of middle page
      current_value = page_get_int_at(middle_page, page_current_pos);
      //Discard left half of pages
      if(search_value > current_value){
        lowest_page_number = middle_page_number + 1;
      }
      //Discard right half of pages
      else{
        highest_page_number = middle_page_number;
      }
    }
  }

  printf("Value is not in table\n");
  return 0;
}

static int find_record_int_val(record r, schema_p s, int (*op) (int, int), int val) 
{
  page_p pg = get_page_for_next_record(s);
  if (!pg) return 0;
  int pos, rec_val;
  for (; pg; pg = get_page_for_next_record(s)) {
    ...
    rec_val = page_get_int_at (pg, pos);
    if ((*op) (val, rec_val)) {
...
-binary = 1 if you want to test binary search for "=" operation
-binary = 0 if you want to test linear search for "=" operation*/
tbl_p table_search(tbl_p t, char const* attr, char const* op, int val, int binary) 
{
  if (!t) return 0;

  int (*cmp_op)() = 0;

  //CALL DIFFERENT FUNCTIONS BASED ON USER OPERATION
  //If select operation is: "="
  if(strcmp(op, "=") == 0){
    cmp_op = int_equal; //Set pointer to cmp_op to the function: "int_equal"
  }
  ...
  //Perform binary search for "equal" operation
  if(strcmp(op, "=") == 0 && binary == 1){
    binary_search(rec, s, cmp_op, val);
    put_record_info(DEBUG, rec, s);   //put info from record into buffer "DEBUG"
    append_record(rec, res_sch);      //Append record to the table file
  }

  //Perform linear search for "equal" operation
  else if(strcmp(op, "=") == 0 && binary == 0){
    find_record_int_val(rec, s, cmp_op, val);
    put_record_info(DEBUG, rec, s);   //put info from record into buffer "DEBUG"
    append_record(rec, res_sch);      //Append record to the table file
  }

  //If select statement is not "equal" then perform pre-code
  else{
    while(find_record_int_val(rec, s, cmp_op, val)){
