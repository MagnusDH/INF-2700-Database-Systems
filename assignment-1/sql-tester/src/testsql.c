...
/*Skips a character in a string*/
char const* skip_chars(char const* const str, const char ch) {
  if (!str) return 0;
...
/*
Returns 1 if:
  -test directory can be reached
  -test file can be opened
  -test database is provided
  -given database can be opened
  -query file is provided
  -query file can be opened
...
int prepare_test(char const* const test_dir, char const* const test_file_name) {
  ...
  int ret = sqlite3_open(testdb_file_name, &db);
...
int get_next_query( char* const query_name, const size_t query_name_len, char* const expectation, const size_t expectation_len)
{
  if(!spec){
    return 0;
  }

  //Looks for the substring "query" in the main string "spec" 
  spec = strstr(spec, "query:");

  if(!(spec && parse_spec_token("query", query_name, query_name_len))){
    return 0;
  }
...
/*################################################################################*/
//Inpired from SQLite Book, chapter 6, page 210
int callback(void *data, int num_columns, char **values, char **headers)
{  
  int i;
...
int query_exec(char const* const query_name, char* const query_res, const size_t res_len, char* const query_err, const size_t err_len)
{
  char const* query_p = get_query_str(query_name);  //Pointer to query string
  if (!query_p) {                                   //Print error if query string can't be fetched
    snprintf(query_err, err_len,
	     "test \"%s\" not found.",
	     query_name);
    return 0;}

  size_t query_len = strstr(query_p, ";\n") - query_p + 1;  //Length of query
  ...
  */
  char query_copy[query_len];
  memcpy(query_copy, query_p, query_len);   //Copy query string

  char *q_err = query_err;
  sqlite3_exec(db, query_copy, callback, query_res, &q_err);
