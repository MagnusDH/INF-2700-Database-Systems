...
static int find_record_int_val(record r, schema_p s, int offset, int (*op) (int, int), int val) {
  page_p pg = get_page_for_next_record(s);
  if (!pg) return 0;
  int pos, rec_val;
...
/*Returns 1 if search_value is in the given array, 0 otherwise*/
int value_in_array(int array[], int search_value)
{
  for(int i=0; i<100; i++){
...
/*Performs a "nested loop join" and returns a table with common records for two given tables
  for(all pages in left){
    while(not end of left page){
      for(all pages in right){
        while(not end of right page J){
          if(match){
            match++;
          }
        }
...
{
   printf("    NESTED_LOOP_JOIN()\n");

  //Create schema containing a table that is to be returned
  schema_p joined_schema = new_schema("Nested_joined");
  add_field(joined_schema, new_int_field("Joined"));
  
  //Get number of blocks/pages in left and right
  int left_num_pages = file_num_blocks(left->sch->name);
  int right_num_pages = file_num_blocks(right->sch->name);
  
  //Create array to check for duplicates in joined table
  int array[1000000];
  for(int i=0; i<1000000; i++){
    array[i] = -1;
  }
  int array_free_pos = 0;
  
  int match = 0;                        //Counts number of matches
  int left_value;                       //Used to hold value at current position in page  
  int right_value;                      //Used to hold value at current position in page  
  int left_current_pos;                 //Keeps track of position in left table
  int right_current_pos;                //Keeps track of position in right table
  page_p left_page;                     //Holds the current left page
  page_p right_page;                    //Holds the current right page

  //Go through all pages in left file 
  for(int i=0; i<left_num_pages; i++){
    // printf("#####################################################LEFT PAGE NR: %d\n", i);
    //get left page
    left_page = get_page(left->sch->name, i);
    // left_current_pos = 54;  //Set position to first INT placement in table
    left_current_pos = 24;  //Set position to first INT placement in table
    // left_current_pos = 20;  //Set position to first INT placement in table


    page_set_current_pos(left_page, left_current_pos);

    //Go through all variables in left current page
    while(!eop(left_page)){
      //Get left variable
      left_value = page_get_int_at(left_page, left_current_pos);
      // printf("Left value: %d\n", left_value);
      
      //For all pages in right file
      for(int j=0; j<right_num_pages; j++){
        // printf("RIGHT PAGE nr: %d\n", j);

        //Get right page
        right_page = get_page(right->sch->name, j);
        // right_current_pos = 54;
        right_current_pos = 24;
        // right_current_pos = 20;


        page_set_current_pos(right_page, right_current_pos);

        while(!eop(right_page)){
          //Get right variable
          right_value = page_get_int_at(right_page, right_current_pos);

          if(left_value == right_value){
            if(value_in_array(array, right_value) == 0){ //Value is not in array
              array[array_free_pos] = right_value;
              array_free_pos++;
              match++;
              record rec = new_record(joined_schema);
              fill_record(rec, joined_schema, right_value);
              append_record(rec, joined_schema);
              break;
            }
          }

          // right_current_pos += 38;
          right_current_pos += 8;
          // right_current_pos += 4;
  ...
  printf("Nested_loop_join found: %d matches\n", match);
  // table_display(joined_schema->tbl);
  return joined_schema->tbl;
}


/*Performs a "block nested loop join" and returns a table with common records for two given tables
  for(all pages in left)
    for(all pages in right)
      while(not end of left page)
        while(not end of right page)
...
{
  printf("    BLOCK_NESTED_LOOP_JOIN\n");

  //Create schema containing a table that is to be returned
  schema_p joined_schema = new_schema("Block_joined");
  add_field(joined_schema, new_int_field("Joined"));
  ...
  int left_value;
  int right_value;
  int left_current_pos;
  int right_current_pos;
  page_p left_page;
  page_p right_page;
  // record record = new_record(joined_schema);

 
  //Go through all pages in left
  for(int i=0; i<left_num_pages; i++){
    //get left page
    left_page = get_page(left->sch->name, i);

    // //Go through all pages in right
    for(int j=0; j<right_num_pages; j++){
      //get right page
      right_page = get_page(right->sch->name, j);

      // reset page position for left page
      // left_current_pos = 54;
      left_current_pos = 24;
      // left_current_pos = 20;
      ...
      while(!eop(left_page)){
        ...
        //Reset page position for right page
        // right_current_pos = 54;
        right_current_pos = 24;
        // right_current_pos = 20;
        ...
        while(!eop(right_page)){
          ...
          //if(leftValue == rightValue)
          if(left_value == right_value){
            if(value_in_array(&array, right_value) == 0){
              array[array_free_pos] = right_value;
              array_free_pos++;
              match++;
              record test = new_record(joined_schema);
              fill_record(test, joined_schema, right_value);
              append_record(test, joined_schema);
              break;
            }
          }
  ...
  printf("Block_nested_loop_join found: %d matches\n", match);
  return joined_schema->tbl;
}
...
tbl_p table_natural_join(tbl_p left, tbl_p right) 
{
  ...
  tbl_p result_table = 0;

  /* TODO: assignment 3 */
  // result_table = nested_loop_join(left, right);
  result_table = block_nested_loop_join(left, right);

  table_display(result_table);
...
/*
Nested loop join:
-Fetch one page from left and one from right
-Compare first value in left page to ALL values in right page nr.1
-Remove right page and insert page nr.2
-Compare first value in left with ALL values in right page nr.2 
*For one record in left, you need to load all blocks and records from right
-When first record in left is compared with ALL records in right, then repeat the process with second value in left
*COST: (All records in left * All blocks/pages in right) + All blocks/pages in left 



block nested loop join:
-Fetch one page from left and one page from right
-Compare first value in left page1 to all values only on right page1
-Then compare second value in left page1 to all values in right page1
-When all values in left page1 has been compared with all values in right page1:
  -Fetch right page2
  -Compare all values in left page1 to all values in right page2

*COST: (all blocks/pages in left * all blocks/pages in right) + all blocks/pages in left
*/
