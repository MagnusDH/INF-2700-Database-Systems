...
/*Creates a table of a given size and name*/
void create_big_table(char *table_name, int num_records, int odd)
{
  //Create new schema and add it to the only database
  schema_p schema = new_schema(table_name);

  //Add integer field in new schema
  char attr1[MAX_TOKEN_LEN] = "Number";
  add_field(schema, new_int_field(attr1));

  //Put debug message into message line
  put_msg(DEBUG, "Create table name: \"%s\".\n", table_name);
  ...
  //Append odd and even numbers to table
  if(odd == 1){
    for(int i=0; i<num_records; i++){
      // int rand_num = rand() % 100;
      // int_string = itoa(rand_num, value, 10);
      int_string = itoa(i, value, 10);


      //Create new and filled record
  ...
  //Append only even numbers to table
  else if(odd == 0){
    for(int i=0; i<num_records; i++){
      // int rand_num = rand() % 100;
      int_string = itoa(i*2, value, 10);
      // int_string = itoa(rand_num, value, 10);
...
void interpret(int argc, char* argv[]) {
  ...
  //Create a big table from start of program
  create_big_table("t1", 3420, 1);
  create_big_table("t2", 3420 , 0);
